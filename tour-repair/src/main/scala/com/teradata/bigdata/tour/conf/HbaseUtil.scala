...
/**
  ...
  * @Modified By:
  */
class HbaseUtil extends Serializable with TeradataConnect{
  ...
  def getAdmin(conn: Connection) = {
    this.init()
    conn.getAdmin
  }

  def createTable(conn: Connection, tableName: String, splitKeys: Array[Array[Byte]]): Unit = {
    val table = new HTableDescriptor(TableName.valueOf(tableName))
    table.addFamily(new HColumnDescriptor("0"))
    getAdmin(conn).createTable(table, splitKeys)
  }

  def createTable(conn: Connection, tableName: String): Unit = {
    val table = new HTableDescriptor(TableName.valueOf(tableName))
    table.addFamily(new HColumnDescriptor("0"))
    getAdmin(conn).createTable(table)
  }
  ...
  def putResultByKeyList(conn: Connection, tableName: String, keyValueList: List[(String, String)]): Unit = {
    val table = conn.getTable(TableName.valueOf(tableName))
    try {
      val muList = new ListBuffer[Put]
      keyValueList.foreach(kv => {
        ...
        muList.add(
          new Put(rowKey.getBytes)
            .setWriteToWAL(false)
            .addColumn("0".getBytes(), "0".getBytes(), value.getBytes())
        )
      })
      table.put(muList)
    } finally {
  ...
  def getSelectLacCiAllRows: collection.Map[String, String] = {
    val conn = interface177Connect()
    val newMap: collection.mutable.Map[String, String] = new scala.collection.mutable.HashMap
    ...
    while (scenicResult.next()) {
      ...
      newMap.update(lacCellId, sceneryId)
    }
