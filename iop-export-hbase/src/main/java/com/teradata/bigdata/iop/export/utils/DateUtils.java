...
public class DateUtils {
    ...
    public static boolean isDate(String date){
        ...
        if(!date.matches("(?m)\\d{8}")){
            return  false;
        }
        try {
            Date dateP = dateFormat.parse(date);
            String date1 = dateFormat.format(dateP);
    ...
    //获取X日之前日期
    public static String getDateBeforeXdays(int x) {
        Calendar c = Calendar.getInstance();
        c.add(Calendar.DATE, -x);
        String beforeDate = dateFormat.format(c.getTime());
        return beforeDate;
    }

    //获取X月之前的月份
    public static String getMonthBeforeXMonths(int x) {
        Calendar c = Calendar.getInstance();
        c.add(Calendar.MONTH, -x);
        String beforeMonth = monthFormat.format(c.getTime());
        return beforeMonth;
    }
...
//        System.out.println(getCurrentDate());
        System.out.println(getMonthBeforeXMonths(3));
        System.out.println(getDateBeforeXdays(10));
        System.out.println(DateUtils.getDateBeforeXdays(1));
    }
